...
func (n *node) applyCommitted(proposal *pb.Proposal, key uint64) error {
	...
	case proposal.UpdateExtSnapshotStreamingState != nil:
		// Handle UpdateExtSnapshotStreamingState proposal with two distinct scenarios:
		...
		if proposal.UpdateExtSnapshotStreamingState.Start {
			x.ExtSnapshotStreamingState(true)
			return nil
		}

		if proposal.UpdateExtSnapshotStreamingState.DropData {
		...
		if proposal.UpdateExtSnapshotStreamingState.Finish && x.IsExtSnapshotStreamingStateTrue() {
